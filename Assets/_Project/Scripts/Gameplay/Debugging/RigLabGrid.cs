...
{
    /// <summary>
    /// RigLab 바닥 격자 — 거리감 확인용 GL 라인 (Docs/06 §4.1). 에디터 전용 씬이라
    /// 런타임 비용은 고려하지 않는다. 1m 간격, 5m마다 밝은 선.
    /// </summary>
    public sealed class RigLabGrid : MonoBehaviour
    {
        [SerializeField] private int _halfExtent = 20;
        [SerializeField] private Color _minorColor = new(1f, 1f, 1f, 0.08f);
        [SerializeField] private Color _majorColor = new(1f, 1f, 1f, 0.25f);
