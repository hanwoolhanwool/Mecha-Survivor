...
| **RigLab.unity** | 리그 실험실 — 리그 프로필(마운트/총구) 조정·저장 + 애니 강제 재생 | 무기 장착 위치·발사 위치·본 추종 확인 (`06_RigLab.md`) |
...
본편 조작과 무관한 독립 씬 — 우클릭 드래그 회전 / 휠 클릭 드래그 팬(시점 이동) / 휠 줌 궤도 카메라.
**우측 UI 패널**로도 전 기능(대상/항목 선택·±넛지·수치 직접 입력·발사·저장·애니)을
마우스로 조작할 수 있다. 수치 필드 타이핑 중에는 핫키가 정지된다 (오발동 방지 —
버튼을 누르거나 빈 곳을 클릭하면 복귀).

| 키 | 동작 |
|---|---|
| Tab | 캐릭터 탭 ↔ 적 탭 |
| ← / → | 대상 순환 (조정 선택이 **없을 때만**) |
| W | 장착 무기 순환 — 고른 무기의 모델만 남긴다(본편 표시 규칙). 기본은 「전체 표시」라 같은 손 무기끼리 겹쳐 보이므로, 그립을 볼 땐 먼저 무기를 고를 것 |
| [ / ] | 마운트/총구 순환 선택 (선택 시 씬 뷰 Selection도 따라감 — 씬 뷰 핸들 병용) |
| 화살표 / PgUp·PgDn | 선택 항목 이동 X·Z / Y (Shift=회전, Ctrl=미세, Alt+PgUp/Dn=스케일) |
| R | 선택 항목을 프로필 값으로 리셋 |
| F | 시험 발사 — 플레이어 탭: 실무기 프리팹 경로(총구 전방으로), 적 탭: EnemyData 투사체 |
| S | **프로필(SO)에 저장** — Play 중 저장해도 유지됨. HUD의 「● 미저장 변경」 표시 확인 |
| 1~5 | FlyIdle / Fly / GroundIdle / Walk / Dash (2·4·5 재누름 = 8방 순환) |
| 6 / 7 / 8 | 사격 유지 토글 (경화기/발사기/중화기 그룹) |
| 9 / 0 | 피격 / 강피격 |
| P | **포즈 클립 순환** (없음 → 클립들 → 없음). AC 밖의 전시용 포즈를 PlayableGraph로 직접 재생 — 현재 `Mecha_Pose_HeroLunge`(히어로 런지, Docs/07). 1~5·6~8·9·0 아무 키나 누르면 포즈에서 자동 이탈 |
| = / - | 재생 속도 0.1×~2× |
| H | 우측 UI 패널 숨김/표시 |
...
- [ ] 캐릭터/적 탭 전환·대상 순환 시 이전 대상이 완전히 회수되는가 (잔존 모델 0)
- [ ] 마운트 조정 → S 저장 → Stop → 재 Play 시 저장 값으로 재구성되는가
- [ ] 랩에서 저장한 총구가 **Game 씬**에서 반영되는가: 레이저 캐논 장착 → 펄스가
  레일건 포신 끝에서 나가는가 (등의 트윈 캐논은 missile_pod 총구)
- [ ] 애니메이션 강제 재생(1~5) 중 마운트·총구가 본을 따라 움직이는가
  — 특히 **손 마운트는 사격 자세(6/7/8)에서 확인 필수**
- [ ] P 포즈 재생 → 다시 P(또는 1~5)로 해제하면 **AC 상태로 정상 복귀**하는가
  (포즈에 얼어붙어 있으면 그래프가 안 끊긴 것). 포즈 중에도 무장이 본을 따라오는가
- [ ] 시험 발사(F)의 궤적이 총구 기즈모 화살표 방향과 일치하는가
- [ ] 미저장 변경(●) 표시 후 S로 사라지는가 / Stop 전 저장을 잊으면 조정이 소실되는가(정상)
- [ ] Game·CityMap 두 씬의 장착 표시가 동일한가 (프로필 단일 소스 — 씬별 차이가 나면 버그)
...
| RigLab에서 스폰한 기체가 **누워 있음** | 게임 뷰가 실제로 렌더되지 않으면 Animator 기본 컬링이 포즈 갱신을 멈춤 — 컨트롤러가 `AlwaysAnimate`로 강제하므로 재발 시 RigLabController 경유 없이 직접 Build했는지 의심. FBX 루트 -90° 자체는 빌더가 로컬 identity 스폰으로 처리 |
| RigLab 조정 값이 Stop 후 사라짐 | 앵커 트랜스폼은 씬 임시 오브젝트 — **S로 프로필(SO)에 저장해야 유지**된다. HUD ● 표시가 남아 있으면 미저장 |
