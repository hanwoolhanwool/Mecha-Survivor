...
### B3. 대각 대쉬 전용 클립 (총 38클립화) ✅ 완료 (2026-07-29)
...
- **다리 규약**: 07 §8 aim2 + §10-2-1 pole (L/R 트레일 다리만 명시 오버라이드).
  **F는 양다리 이등분 pole 그대로** — §10-2-1의 "F에도 적용하면 좌 18/18/18·우 23/34/46"은
  ...
  ※ 전시용 F 포즈 클립은 계속 무보정(aim2 이전) 원본 유지(소급 여부 미결).
- **지상/비행 동일 내용**: 지상 대쉬 = 순간 부양 기동(§1)이므로 공중 런지 포즈를 그대로 수용.
  차별화(지상 저고도 등)는 플레이 확인 후 필요 시.
- **대각(FL/BL/FR/BR) 1차 유지**: 기존 절차 포즈 그대로. 대각 히어로화는 07 §10-2의
  대각 포즈 결정과 함께 별도 진행 (두 히어로 포즈의 50/50 블렌드는 다리 실루엣이 뭉개질
  위험이 있어 채택하지 않음).
